...
{
	__set_FAULTMASK(1);//开关所有中断
	NVIC_SystemReset();//系统恢复
}
...
u8 mpu_dmp_init(void)
{
    ...
    if(mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL))//配置FIFO保存陀螺仪数据和加速计的原始数据                                                 
			return 3;                                         
    if(mpu_set_sample_rate(DEFAULT_MPU_HZ))            //设置MPU采样率，这里为100Hz（当DMP没用使用时，则MPU的采样率由此设置决定，当使用了DMP时，则有DMP的输出频率决定采样率）
			return 4;
