...
extern u8 rxbuf[10]; // 接收飞控数据缓存

u8 Lock_State;//飞控是否解锁状态
...
//	ADC_ConvertedValue[0] = ADC_ConvertedValue[0];
	...
	if((accelerator-(ADC_ConvertedValue[1]/82.0+100))>=50.0){
		accelerator -= 50;											//当快速拉低油门时，则进行一个迅速减小动力的措施
	}else{
		accelerator = ADC_ConvertedValue[1]/82.0+100;					//正常速度拉低油门时，则可以按照正常运行即可，注意加大油门时不会受此机制的影响
	}
...
{
	Lock_State=rxbuf[1];     //飞控是否解锁状态   0：加锁  1：解锁
	if(Lock_State)LED_On(LED1);
